...
def adopt_move(src: Path, name: str) -> str:
...
def op_import(b):
    ...
    parts = []
    if adopted:
        parts.append(f"已收编 {len(adopted)} 个({'、'.join(adopted)}),原位置留了链接")
    if copied:
        parts.append(f"已复制导入 {len(copied)} 个({'、'.join(copied)}),原目录不动")
    out = ";".join(parts) if parts else "没有导入任何技能"
    if notes:
        out += f";{'; '.join(notes)}"
    if failed:
